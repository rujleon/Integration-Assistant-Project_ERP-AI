...
with col_left:
    try:
        ...
        with c2:
            ...
            retour_clicked = st.button(" ⬅️ ", width="stretch")
            if retour_clicked:
...
with col_right:
    ...
    assistant_open = st.toggle("🔓 Activer l’assistant")
    ...
    if assistant_open:
        ...
        # Déclenchement automatique avec Entrée ou bouton
        send_triggered = st.button("Envoyer", key="assistant_btn") or (
            question and st.session_state.get("last_question") != question
        )
            ...
            with st.spinner("💡 Génération et exécution de la requête SQL..."):
                ...
                if not sql_query.lower().startswith("select"):
                    ...
                    try:
                        ...
                        # Résumé automatique en phrase naturelle
                        summary_prompt = f"""
        ...
        st.info("🔒 Assistant désactivé. Activez-le pour poser vos questions SQL.")
